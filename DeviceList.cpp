...
void	query_input_connection (IDeckLink* deckLink);
void	switch_input_connection (IDeckLink* deckLink, BMDVideoConnection connection);
...
//command arguments
	...
	if ( argc > 2){
			...
			while (deckLinkIterator->Next(&deckLink) == S_OK){
				if(i == atoi(argv[1])){
						if(std::string(argv[2]) == "attr")print_attributes(deckLink);
						else if(std::string(argv[2]) == "out")print_output_modes(deckLink);
						else if(std::string(argv[2]) == "in")print_input_modes(deckLink);
						else if(std::string(argv[2]) == "list")print_capabilities(deckLink);
						else if(std::string(argv[2]) == "get")query_input_connection(deckLink);
						else if(std::string(argv[2]) == "sdi")switch_input_connection(deckLink, bmdVideoConnectionSDI);
						else if(std::string(argv[2]) == "hdmi")switch_input_connection(deckLink, bmdVideoConnectionHDMI);
				}
				deckLink->Release();
				i++;
			}
		...
		printf(
		"modified by haydent www.httech.com.au\n\n"
...
void	query_input_connection (IDeckLink* deckLink)
{
	//written by hayden www.httech.com.au to query between sdi/hdmi ports on deccklink mini
	...
	if (result == S_OK)
	{
		if (ports & bmdVideoConnectionSDI)printf("SDI");

		if (ports & bmdVideoConnectionHDMI)printf("HDMI");
	}
	printf("\n\n");
...
void	switch_input_connection (IDeckLink* deckLink, BMDVideoConnection connection)
{
	//written by hayden www.httech.com.au to switch between sdi/hdmi ports on deccklink mini
	IDeckLinkConfiguration*		deckLinkConfiguration = NULL;
